 ...
 * Title: TecladoJS
 * Version: v0.4.1
 * Description: Fork de la librería KeyboardJS para manejar eventos de teclado. Solo que se ha traducido al castellano con motivos educativos.
 * library.
 * Author: Robert Hurst.
 *
...
(function(context, factory) {
	...
	var teclado = {}, locales = {}, locale, map, macros, activeKeys = [], bindings = [], activeBindings = [],
	...
	teclado.enable = enable;
	teclado.disable = disable;
	teclado.activeKeys = getActiveKeys;
	teclado.cuando = createBinding;
	teclado.limpiar = removeBindingByKeyCombo;
	teclado.limpiar.key = removeBindingByKeyName;
	teclado.locale = getSetLocale;
	teclado.locale.register = registerLocale;
	teclado.macro = createMacro;
	teclado.macro.remove = removeMacro;
	teclado.key = {};
	teclado.key.name = getKeyName;
	teclado.key.code = getKeyCode;
	teclado.combo = {};
	teclado.combo.active = isSatisfiedCombo;
	teclado.combo.parse = parseKeyCombo;
	teclado.combo.stringify = stringifyKeyCombo;
	return teclado;
	...
	/**
	 * Exits all active bindings and disables teclado
	 */
	function disable() {
		reset();
	...
	function getSetLocale(localeName) {
		...
		if(localeName) {
			...
			if(!locales[localeName]) { throw new Error('Cannot set locale to ' + localeName + ' because it does not exist. If you would like to submit a ' + localeName + ' locale map for teclado please submit it at https://github.com/RobertWHurst/KeyboardJS/issues.'); }
